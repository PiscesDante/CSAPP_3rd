...
*   现在有两个方法将函数引用进编写的源文件：
    *   选择 1 ：将所有的函数编写到一个巨大的单个文件中，然后程序员就必须将整个对象文件链接到程序中；这样就会造成大量的空间和时间浪费。
    *   选择 2 ：将每个函数放在单独的源文件中，程序员将合适的文件链接到程序中；这个做法将提升效率，但是对于使用这个库的程序员就相当麻烦了；

## 旧时代的解决办法：静态库

静态库是一个 `.o` 文件的集合，其中每个 `.o` 文件都包含一个函数。

也就是说，这个集合也许包含很多的函数，也包含很多的对象文件，但是我只需要将这个静态库引用进来，至于我使用哪些函数是我的事情，链接的事情就交给链接器，链接器根据我的使用情况来进行链接函数即可。

## 静态库的使用

### 链接器处理外部引用的算法

*   扫描命令行中所有的 `.o` 文件和 `.a` 文件；
*   在扫描的同时，创建并维护一个未解析的引用列表；
*   对于遇到的每个新的 `.o` `.a` 文件和 `obj` 冲突的话，链接器就会尝试使用未解析列表中的引用进行解析；
*   当扫描结束之后未解析列表中如果非空，那么报错。

### 关键问题

*   链接器将尝试在命令行上从左到右解析这些引用，所以命令行中文件的排列顺序是非常讲究的。
*   潜规则：将库放在命令行的最后。
...
*   每个可执行文件中都要保存一份用到的库的副本；
*   每个可执行文件都运行一份所引用库的副本；
*   很小的改动都需要每个程序进行重新链接；

### 现代解决方案：共享库

*   代码和数据将动态的加载到应用中，程序可以在运行时再决定应当使用哪些共享库中的函数。
*   也被叫做动态链接库，文件后缀 `.dll` 。

**<u>动态链接是在程序第一次加载和运行期间发生的（加载时链接）</u>** 。

**<u>动态链接也会在程序已经运行起来的情况下发生（运行时链接）</u>** 。
...
int main()
{
    ...
    /* Dynamically load the shared library that contains add_vec */
    /* 第二个参数的意思是只要返回一个引用即可 */
    dynamic_library_handle = dlopen("./libvector.so", RTLD_LAZY);
    if (!dynamic_library_handle) {
        ... /* ERROR */
    } else {
        add_vec = dlsym(dynamic_library_handle, "add_vec"); /* 将函数引用出来 */
        /* 现在就可以像使用一般函数一样来使用 add_vec */
        add_vec(x, y, z, 2);
        ...
    }
}
...
*   链接是一种允许使用多个可重定向文件来构建程序的技术；
*   链接可以在程序的生命周期中的任意时刻发生：
    *   编译期（当程序编译的时候）；
    *   加载期（当程序加载进入内存的时候）；
    *   运行期（当程序已经运行起来的时候，上面的例子就是）；
*   对于链接的理解可以让你避免非常麻烦的错误，从而成长为一个更好地程序员；

## 案例学习：库打桩技术

*   库打桩技术： **<u>一种可以使程序员拦截任意函数调用的强大技术</u>** 。
*   打桩技术可以出现在：
    *   编译期；
    *   链接期；
    *   加载 & 运行期；
...
void* my_malloc(size_t size)
{
    ...
    printf("malloc(%d) => %p", size, memory_address);
    return memory_address;
}

void my_free(void* memory_address)
{
    free(memory_address);
    printf("%p has been free");
}

#endif
```

定义好以上的函数之后，就在实际的文件中进行预处理：

```C
#define malloc(size) my_malloc(size)
#define free(memory_address) my_free(memory_address)
...
*   编译期打桩：使用自定义的包装函数来修饰本身需要调用的函数，自定义的包装函数拥有原函数的核心功能的返回值，但是其中有更多的自定义功能；
*   链接期打桩：使用特殊的手段将原本解析到原函数的引用解析到自定义的包装函数上来；
*   加载/运行期打桩：使用动态链接的手段来调用动态链接库中的原函数进行包装即可；
